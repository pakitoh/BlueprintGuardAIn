...
def send_webhook(mode: str):
    ...
    try:
        ...
        print(
            f"{status_color} Status: {response.status_code} | Repo: {payload['repository']['full_name']}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate GitHub webhooks with random data."
    )
    parser.add_argument(
        "--mode",
        choices=["push", "pr", "random"],
        default="random",
    ...
    parser.add_argument(
        "--count",
        type=int,
        help="Number of requests to send (omit to run indefinitely)",
    )
    parser.add_argument(
        "--delay",
        type=float,
